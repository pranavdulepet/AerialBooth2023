...
def run_diffusion_experiment(init_image_path, output_dir, prompt, device='cuda'):
    ...
    pipe = DiffusionPipeline.from_pretrained(
        ...
        cache_dir='dir_name',
        scheduler=DDIMScheduler(beta_start=0.00085, beta_end=0.012, beta_schedule="scaled_linear", clip_sample=False, set_al$
    ).to(device)
    ...
    for view_mode in view_modes:
        ...
        for i in range(5):  
            res = pipe(alpha=0.1, guidance_scale=7.5, num_inference_steps=50, mi_lr=0, eval_prompt=eval_prompt)
            image = res.images[0]
