...
def run_diffusion_experiment(init_image_path, zero_output_path, output_dir, prompt, device='cuda'):
    ...
    pipe = DiffusionPipeline.from_pretrained(
        ...
        custom_pipeline='./models/aerialbooth_sum_exp1',
        cache_dir='dir_name',
    ...
    image_hom = main_image.resize((512,512))
    ...
    for view_mode in view_modes:
        ...
        res = pipe(alpha=0.1, guidance_scale=7.5, num_inference_steps=50, mi_lr=1e-6, eval_prompt=eval_prompt, image_hom=image_hom)
        image = res.images[0]
        image.save(f'{output_dir}/{view_mode}/{view_mode}_architectures34.png')
        print(f"{view_mode} output saved at {output_dir}/{view_mode}/{view_mode}_architectures34.png")
